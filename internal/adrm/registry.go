...
func commandRegistry() []CommandInfo {
	return []CommandInfo{
		...
		{
			Name:        "accept",
			Purpose:     "Mark an ADR as accepted.",
			Safety:      "write: updates one ADR metadata block and appends history",
			Selectors:   []string{"--id", "--reason"},
			Examples:    []string{"adrm accept --id ADR-0001 --reason \"Approved by the team.\" --dry-run", "adrm accept --id ADR-0001 --reason \"Approved by the team.\""},
			NextCommand: []string{"adrm show --id ADR-0001"},
			Mutating:    true,
			HasDryRun:   true,
		},
		{
			Name:        "reject",
			Purpose:     "Mark an ADR as rejected.",
			Safety:      "write: updates one ADR metadata block and appends history",
			Selectors:   []string{"--id", "--reason"},
			Examples:    []string{"adrm reject --id ADR-0001 --reason \"Chose a different approach.\" --dry-run", "adrm reject --id ADR-0001 --reason \"Chose a different approach.\""},
			NextCommand: []string{"adrm show --id ADR-0001"},
			Mutating:    true,
			HasDryRun:   true,
		},
